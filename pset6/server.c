...
char* indexes(const char* path)
{
    ...
    char *new_php_path =  malloc(len + strlen(php) + 1 ); /* one for extra char, one for trailing zero */
    char *new_html_path = malloc(len + strlen(html) + 1 ); /* one for extra char, one for trailing zero */
    
    strcpy(new_php_path, path);
    strcat(new_php_path, php);
    
    strcpy(new_html_path, path);
    strcat(new_html_path, html);

    if (access(new_php_path, R_OK) != -1)
    {
        return new_php_path;
    } 
    else if (access(new_html_path, R_OK) != -1)
    {
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    BYTE* inbuffer  = malloc(1*sizeof(BYTE));
    BYTE* outbuffer = NULL;
    ...
    do
    {   
        fread(inbuffer, sizeof(BYTE), 1, file);
        bytes_read++; 
        tempbuffer = realloc(outbuffer, bytes_read*sizeof(BYTE));

        if (tempbuffer != NULL) 
        {
            outbuffer = tempbuffer;
            outbuffer[bytes_read - 1] = *inbuffer;
        } 
        else 
        {
            free(inbuffer);
...
const char* lookup(const char* path)
{
    if (strcasestr(path, ".jpg"))  { return "image/jpeg"; }
    if (strcasestr(path, ".html")) { return "text/html"; }
    if (strcasestr(path, ".css"))  { return "text/css"; }
    if (strcasestr(path, ".gif"))  { return "image/gif"; }
    if (strcasestr(path, ".ico"))  { return "image/x-icon"; }
    if (strcasestr(path, ".js"))   { return "text/javascript"; }
    if (strcasestr(path, ".php"))  { return "text/x-php"; }
    if (strcasestr(path, ".png"))  { return "image/png"; }
    return NULL;
}
...
bool parse(const char* line, char* abs_path, char* query)
{
    ...
    for(int i = 0; i < strlen(request_target); i++) 
            request_target[i] = tolower(request_target[i]);
    ...
    if (strchr(request_target, 63) == NULL) 
    {
        strcpy(query, "");
    ...
    {
        char* ab = NULL;
        ab = malloc((strlen(request_target) + 1));
        ab = strtok(request_target,"?");

        strcpy(abs_path, ab);
        
        char* qu = NULL;
        qu = malloc((strlen(request_target) + 1));
        qu = strtok(NULL,"?");
